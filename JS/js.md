...
-   Objects are Non Primitive Datatypes, these are key value pairs: `let item = {H: 1,};`
-   Type Conversion
...
-   Spread in function calls: The spread (...arr) syntax allows an iterable to be expanded in places where function calls (zero or more arguments) or array literals (elements) are expected
...
-   resolve is catch by `.then()` and reject is catch by `.catch()`
-   Three States:
    -   Pending: Initial state, neither fulfilled nor rejected.
    -   Fulfilled: The operation completed successfully.
    -   Rejected: The operation failed.
-   Promises allow you to chain multiple asynchronous operations together using .then() methods, making the code more readable and avoiding "callback hell".
    ```js
    const res = fakeRequestPromise("helloworld.com/a1"); //it returns an unfulfilled promise which upon fulfillment performs 'then'(if successful) or 'catch'(if error)
    res.then(() => {
        return fakeRequestPromise("helloworld.com/a2");
    })
        .then(() => {
            console.log("Yes3");
        })
...
-   it returns a promise which will either resolve(returned from async func) or reject(error thrown by async func)
